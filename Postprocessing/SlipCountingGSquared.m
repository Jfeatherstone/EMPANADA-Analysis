...
for i=1: length(lunarGravityTrials)
   ...
   data = abs(lunarGravityTrials(i).results.averageGSquared - mean(lunarGravityTrials(i).results.averageGSquared(1:10)));
...
for i=1: length(martianGravityTrials)
   % We count trials by using the localmax function on the brightness derivative
   data = abs(martianGravityTrials(i).results.averageGSquared - mean(martianGravityTrials(i).results.averageGSquared(1:10)));
...
for i=1: length(microGravityTrials)
   % We count trials by using the localmax function on the brightness derivative
   data = abs(microGravityTrials(i).results.averageGSquared - mean(microGravityTrials(i).results.averageGSquared(1:10)));
...
for i=1: length(earthGravityTrials)
    % We count trials by using the localmax function on the brightness derivative
    data = abs(earthGravityTrials(i).results.averageGSquared - mean(earthGravityTrials(i).results.averageGSquared(1:10)));
...
if saveFigs
    saveFileNameNoExtension = 'Lunar-SlipCounting-GSquared';
...
if saveFigs
    saveFileNameNoExtension = 'Martian-SlipCounting-GSquared';
...
if saveFigs
    saveFileNameNoExtension = 'Micro-SlipCounting-GSquared';
...
if saveFigs
    saveFileNameNoExtension = 'Earth-SlipCounting-GSquared';
...
xlabel('Probe speed [mm/s]');
ylabel('Number of slipping events');
%title(['Slipping events vs. probe speed (micro) (', num2str(microMinPeakHeight), ')']);
...
if saveFigs
    saveFileNameNoExtension = 'All-SlipCounting-GSquared';
    printfig(5, saveFileNameNoExtension);
    savePDF('All-SlipCounting-GSquared')
end
