function MaxLoadGSquared(matFileContainingTrials, saveFigs)
...
ylabel('Maximum GSquared [a.u.]');
title('Maximum Load via GSquared (Lunar)');
...
if saveFigs
    saveFileNameNoExtension = 'Lunar-MaxLoad-GSquared';
...
xlabel('Probe speed [mm/s]');
ylabel('Maximum GSquared [a.u.]');
title('Maximum Load via GSquared (Martian)');
% A string plus an array gives a string array with each element the string
% plus the original element appended at the end
% ie. "Day " + [1, 2, 3] = ["Day 1", "Day 2", "Day 3"]
...
if saveFigs
    saveFileNameNoExtension = 'Martian-MaxLoad-GSquared';
...
xlabel('Probe speed [mm/s]');
ylabel('Maximum GSquared [a.u.]');
title('Maximum Load via GSquared (Micro)');
legend("Day " + (min(microDays):max(microDays)));

if saveFigs
    saveFileNameNoExtension = 'Micro-MaxLoad-GSquared';
...
yyaxis right
ylabel('Maximum \langle G^2 \rangle');
...
if saveFigs
    saveFileNameNoExtension = 'All-MaxLoad-GSquared';
    printfig(5, saveFileNameNoExtension);
    savePDF('All-MaxLoad-GSquared')
end
end
