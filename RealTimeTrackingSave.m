...
for trialNum=1: length(trials)
    ...
    video = VideoReader(trials(trialNum).fullPath);
    ...
    saveFileName = [settings.avi_savepath, 'Day', char(trials(trialNum).day), '-', char(trials(trialNum).gravity), '-', char(trials(trialNum).speed), 'mms-RealTime.avi'];
